...
public:
    ...
    CComPtr<IVMRSurfaceAllocatorNotify> m_SAN;      // allows to advise and unadvised custom AP, as well as  
    ...
    // basic operations
    HRESULT         OpenMovie();    
    HRESULT         PlayMovie(BOOL bPause = FALSE);                    
    HRESULT         PauseMovie();                   
    HRESULT         StopMovie();                    
    HRESULT         CloseMovie();                   
...
//    CCritSec    m_AppImageLock;
    ...
    int         m_nsize;                        // number of movies in the list

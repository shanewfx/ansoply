...
*/
	...
	if( !m_pEffect )
	{
		CEffect * peffectNew = new CEffect(eEffectDefault);
		peffectNew->Initialize(&m_movieList, 1000, -1, 500);
		m_pEffect = peffectNew;
	}
...
/*	POSITION pos = m_videoGroups.GetHeadPosition();
	while ( pos )
	{
		...
		CVideoObject * pMovie = pVideoGroup->GetCurVideoObj();
		
		if (!pMovie)
			continue;

		if (pVideoGroup->GetObjectID() == m_lSelectGroupID)
			bSelectedChannel = TRUE;
		else
			bSelectedChannel = FALSE;

		ULONG cX, cY, uWidth, uHeight, uAlpha;
		pVideoGroup->GetVideoPosAndSize(&cX, &cY, &uWidth, &uHeight);
		...
		hr = m_pD3DHelper->RenderFrame( pMovie->m_lpDDTexture, 
			pMovie->m_Vcur, 
			m_rcDst, 
			rt,
